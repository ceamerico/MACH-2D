...
!! \brief      MACH-2D
!!                 Version: 5.10.2
!!             Last update: 23/08/2020 by Guilherme Bertoldo
!!
program main
   ...
   do it = 1, itmax
      ...
      if ( kflow == INTERNAL_FLOW ) then
         ...
         if ( modturb == 1 ) then
            ...
            call get_eddy_viscosity_blomax(nx, ny, x, y, xp, yp, xk, yk, Jp, Jn, vln &
            ,                         ro_av, ron_av, u_av, v_av, vtp) ! Last one is output
            ...
            call get_thermal_conductivity_at_faces(thermomodel, nx, ny, Tbn, Tbs, Tbe, Tbw, kp, ke, kn)
